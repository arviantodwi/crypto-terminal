...
export function calcPivotRange(c2: OhlcCandle): GroupFormulaResult {
  return { name: 'R1', sl_eligible: true, value: c2.candle_range };
}
 ...
 */
export function calcFullSwing(c1: OhlcCandle, c3: OhlcCandle): GroupFormulaResult {
  const value = Math.abs(c1.pct_change) + Math.abs(c3.pct_change);
  return { name: 'R2', sl_eligible: true, value };
}
...
export function calcReversalStrength(c3: OhlcCandle): GroupFormulaResult {
  ...
  return { name: 'R3', sl_eligible: false, value };
}
 ...
 */
export function calcRejectionMagnitude(c1: OhlcCandle, c3: OhlcCandle): GroupFormulaResult {
  const value = Math.max(c1.candle_range, c3.candle_range);
  return { name: 'R4', sl_eligible: true, value };
}
...
export function calcBodyConflict(
  ...
  return { name: 'R5', sl_eligible: true, value };
}

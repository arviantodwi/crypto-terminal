 ...
 */
export function calcMaxRange(c1: OhlcCandle, c2: OhlcCandle, c3: OhlcCandle): GroupFormulaResult {
  const value = Math.max(c1.candle_range, c2.candle_range, c3.candle_range);
  return { name: 'P1', sl_eligible: true, value };
}
...
export function calcAbsoluteVolatility(
  ...
  const value = (Math.abs(c1.pct_change) + Math.abs(c2.pct_change) + Math.abs(c3.pct_change)) / 3;
  return { name: 'P2', sl_eligible: true, value };
}
 ...
 */
export function calcChaosScore(c1: OhlcCandle, c2: OhlcCandle, c3: OhlcCandle): GroupFormulaResult {
  const mean = (c1.pct_change + c2.pct_change + c3.pct_change) / 3;
  const variance =
    ((c1.pct_change - mean) ** 2 + (c2.pct_change - mean) ** 2 + (c3.pct_change - mean) ** 2) / 3;
  const value = Math.sqrt(variance);
  return { name: 'P3', sl_eligible: true, value };
}
...
export function calcWickDominance(
  ...
  return { name: 'P4', sl_eligible: true, value };
}
...
export function calcTotalExposure(
  ...
  return { name: 'P5', sl_eligible: true, value };
}

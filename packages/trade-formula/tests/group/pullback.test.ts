...
const c2: OhlcCandle = {
  open: 101.2, high: 101.3, low: 100.5, close: 100.7,
  pct_change: -0.494,
  body_ratio: 0.625,
  candle_range: 0.79,
};
const c3: OhlcCandle = {
  open: 100.7, high: 101.8, low: 100.6, close: 101.5,
  pct_change: 0.794,
  body_ratio: 0.727,
  candle_range: 1.092,
};
...
describe("calcWickDominance (P4)", () => {
  ...
  it("returns 0 when all candles have body_ratio = 1 (no wicks)", () => {
    ...
    const result = calcWickDominance(full_body, { ...c2, body_ratio: 1 }, { ...c3, body_ratio: 1 }, 1.0);
    expect(result.value).toBeCloseTo(0, 10);
  });
});

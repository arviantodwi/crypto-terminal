...
const c1: OhlcCandle = {
  open: 100, high: 100.5, low: 98.8, close: 99.0,
  pct_change: -1.0,
  body_ratio: 0.625,
  candle_range: 1.2,
};
...
const c3: OhlcCandle = {
  open: 99.2, high: 101.0, low: 99.1, close: 100.8,
  pct_change: 1.613,
  body_ratio: 0.842,
  candle_range: 1.915,
};
...
describe("calcReversalStrength (R3)", () => {
  ...
  it("can return a negative value when c3 is bearish (directional signal)", () => {
    const bearish_c3: OhlcCandle = { ...c3, pct_change: -1.2, body_ratio: 0.7 };
...
describe("calcBodyConflict (R5)", () => {
  it("returns abs(c1.body_ratio - c3.body_ratio) × volatility_proxy", () => {
    ...
    expect(result.value).toBeCloseTo(
      Math.abs(c1.body_ratio - c3.body_ratio) * volatility_proxy,
      5,
    );

...
describe("Step 3 — Pre-computation layer", () => {
  ...
  it("computes volatility proxy ≈ 0.666%", () => {
    expect(calcVolatilityProxy(c1.candle_range, c2.candle_range, c3.candle_range)).toBeCloseTo(0.666, 3);
  });
...
describe("Step 4 — Route discrimination", () => {
  it("routes to Trend (directional_agreement = +3)", () => {
    expect(
      discriminateRoute(directionalAgreement, c1Label, c3Label, m1, m3),
    ).toBe("Trend");
  });
});

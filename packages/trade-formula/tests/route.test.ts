...
describe('discriminateRoute', () => {
  describe('Trend', () => {
    it("returns 'Trend' when directional_agreement is +3", () => {
      expect(discriminateRoute(3, 'up_weak', 'up_medium', 0.347, 0.445)).toBe('Trend');
    });

    it("returns 'Trend' when directional_agreement is -3", () => {
      expect(discriminateRoute(-3, 'down_strong', 'down_weak', -0.9, -0.4)).toBe('Trend');
    });
    ...
    it("returns 'Trend' regardless of momentum scores when agreement is ±3", () => {
      ...
      expect(discriminateRoute(3, 'up_weak', 'up_strong', 0.1, 1.5)).toBe('Trend');
    });
  });
  ...
  describe('Reversal', () => {
    it("returns 'Reversal' when agreement=±1, sign flip, and c3 outperforms c1", () => {
      ...
      expect(discriminateRoute(1, 'down_weak', 'up_strong', -0.2, 0.9)).toBe('Reversal');
    });

    it("returns 'Reversal' for bearish reversal: c1=up, c3=down, c3 outperforms", () => {
      expect(discriminateRoute(-1, 'up_weak', 'down_strong', 0.15, -0.95)).toBe('Reversal');
    });
  ...
  describe('Pullback', () => {
    it("returns 'Pullback' when agreement=±1 and both c1 and c3 are same direction", () => {
      ...
      expect(discriminateRoute(1, 'up_weak', 'up_medium', 0.2, 0.35)).toBe('Pullback');
    });

    it("returns 'Pullback' when agreement=-1 and both c1 and c3 are bearish", () => {
      expect(discriminateRoute(-1, 'down_weak', 'down_medium', -0.2, -0.35)).toBe('Pullback');
    });
    ...
    it("returns 'Pullback' when sign flip but c3 does NOT outperform c1 (failed reversal)", () => {
      ...
      expect(discriminateRoute(1, 'down_strong', 'up_weak', -0.9, 0.15)).toBe('Pullback');
    });

    it("returns 'Pullback' when sign flip and momentum is exactly equal (not strictly greater)", () => {
      expect(discriminateRoute(1, 'down_medium', 'up_medium', -0.5, 0.5)).toBe('Pullback');
    });
  });

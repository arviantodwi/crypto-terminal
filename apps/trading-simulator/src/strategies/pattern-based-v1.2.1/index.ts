...
// ── PatternBasedV1.2.1 strategy runner ────────────────────────────────────────

/**
 * pattern-based-v1.2.1 strategy.
 *
 * Extends v1.2 with a revised downward-adjustment rule. The initial
 * `tpMultiplier` from config is used until both sides of Profit Factor are
 * available. Once PF becomes finite (grossLoss > 0 && grossProfit > 0), the
 * effective multiplier adjusts each trade:
 *   - If current multiplier > PF  → subtract 3% of the current multiplier
 *                                   (i.e. multiplier * 0.97)
 ...
 *
 * Instantiate via `loadStrategy('pattern-based-v1.2.1', db, config)` in
 * loader.ts rather than directly — the loader pre-populates the pattern cache
 * from the DB.
 */
export class PatternBasedV121 implements StrategyRunner {
  readonly name = 'pattern-based-v1.2.1';
